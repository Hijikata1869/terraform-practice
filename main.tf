...
data "aws_iam_policy_document" "ecs_task_execution" {
  ...
  statement {
    effect = "Allow"
    acitons = ["ssm:GetParameters", "kms:Decrypt"]
    resource = ["*"]
  }
}
...
resource "aws_cloudwatch_log_group" "for_ecs_sheduled_tasks" {
...
rerource "aws_db_instance" "example" {
  identifier = "example"
  engine = "mysql"
  engine_version = "5.7.25"
  instance_class = "db.t3.samll"
  allocated_storate = 20
  max_allocated_strage = 100
  storage_type = "gp2"
...
resrouce "aws_elastichache_subnet_group" "example" {
  name = "example"
  subnet_ids = [aws_subnet.private_0.id, aws_subnet.private_1.id]
}

resource "aws_elasticache_replication_group" "example" {
  replication_gropu_id = "example"
...
module "codepipeline_role" {
  source = "./iam_role"
  name = "codepipeline"
  identifier = "codepipiline.amazonaws.com"
...
resource "aws_codepipeline" "example" {
  name = "example"
  role_arn = module.codepipeline_role.iam_role.arn
...
resource "aws_codepipeline_webhook" "example" {
  ...
  authentication_configuration {
    secret_token = "VeryRondomStringMoreThan20Byte!"
  }
...
resource "github_repository_webhook" "example" {
  ...
  configuration {
    ...
    secret = "VeryRondomStringMoreThan20Byte!"
    content_type = "json"
    insecure_ssl = false
  }
...
resoruce "aws_s3_bucket" "cloudwatch_logs" {
...
resource "aws_cloudwatch_log_subscription_filter" "example" {
  name = "example"
  log_group_name = aws_cloudwatch_log_group.for_ecs_sheduled_tasks.name
